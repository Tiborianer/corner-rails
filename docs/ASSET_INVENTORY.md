...
The Nightjet N2 set is the next isolated review candidate. Its complete 204.675 m modeled formation is about 238 KB after glTF Transform deduplication/pruning; six reusable module GLBs cover the Taurus, two sleeper layouts, couchette, multifunction car and control/seat car at roughly 37–44 KB each. N2 redraws the Taurus with mirrored geometry-only red and silver sweeps, cab blocks and nose belts derived from the supplied appearance references. It is available only through `?trainLab=nightjet-new-generation`, where Taurus-leading and cab-car-leading pass-through directions can be reviewed separately. The manifest records the nine supplied filenames, but the images remain research-only and are not copied, embedded, served or used as textures. The normal Tier 5 Nightjet remains on its existing `legacy-v1` asset until explicit visual approval.
