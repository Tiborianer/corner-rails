...
| Nightjet review N2 | Private eight-vehicle Taurus 1116 + new-generation Nightjet candidate: two sleepers, three couchettes, multifunction car and control/seat car | `assets/blender/nightjet-new-generation/`, `public/models/train-lab/nightjet-new-generation/` |
...
The Nightjet N2 set is the next isolated review candidate. Its complete 204.675 m modeled formation is about 238 KB after glTF Transform deduplication/pruning; six reusable module GLBs cover the Taurus, two sleeper layouts, couchette, multifunction car and control/seat car at roughly 37–44 KB each. N2 redraws the Taurus with mirrored geometry-only red and silver sweeps, cab blocks and nose belts derived from the supplied appearance references. It is available only through `?trainLab=nightjet-new-generation`, where Taurus-leading and cab-car-leading orientations can be reviewed on the same left-to-right pass-through path. The manifest records the nine supplied filenames, but the images remain research-only and are not copied, embedded, served or used as textures. The normal Tier 5 Nightjet remains on its existing `legacy-v1` asset until explicit visual approval.
