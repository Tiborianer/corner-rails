...
| Nightjet review N1 | Private eight-vehicle Taurus 1116 + new-generation Nightjet candidate: two sleepers, three couchettes, multifunction car and control/seat car | `assets/blender/nightjet-new-generation/`, `public/models/train-lab/nightjet-new-generation/` |
...
The Nightjet N1 set is the next isolated review candidate. Its complete 204.675 m modeled formation is about 235 KB after glTF Transform deduplication/pruning; six reusable module GLBs cover the Taurus, two sleeper layouts, couchette, multifunction car and control/seat car at roughly 37–44 KB each. It is available only through `?trainLab=nightjet-new-generation`. The manifest records the nine supplied filenames, but the images remain research-only and are not copied, embedded, served or used as textures. The normal Tier 5 Nightjet remains on its existing `legacy-v1` asset until explicit visual approval.
